...
public class GeetestCrawlerV2{
    ...
    public static void geetestExecuter(ChromeDriver dv){
            driver = dv;
        try {
            Actions actions = new Actions(driver);
            if(driver.findElements(By.cssSelector("canvas[class='geetest_canvas_bg geetest_absolute']")).size() == 0){
                System.out.println("点选验证码");
                driver.quit();
                return;
            }
            //图一
